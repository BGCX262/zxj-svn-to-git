 ...
 * $Id: RecTree.java,v 1.1.2.12 2006/07/17 16:41:24 mike Exp $
 */
...
public class RecTree extends ZXObject {
    ...
    public zXType.rc initSubNode(ZXBO pobjNode, int pintParentID) throws ZXException {
        ...
        try {
            ...
            if (objParent == null) { 
            	getZx().trace.addError("Unable to retrieve parent node for node subnode, ",
            						 	pobjNode.getDescriptor().getName() + " - " + pintParentID);
            ...
            /**
             * - Same root as parent 
             * - One level higher than parent 
             * - PK of parent as parent 
             * - Hierarchy of parent + <own pk> & /
             */
    ...
    public boolean isValidTreeBO(ZXBO pobjBO) throws ZXException {
        ...
        try {
            ...
            if (pobjBO.getDescriptor().getAttribute("root") == null) return isValidTreeBO; 		// GoTo errExit
            if (pobjBO.getDescriptor().getAttribute("lvl") == null) return isValidTreeBO; 		// GoTo errExit
            if (pobjBO.getDescriptor().getAttribute("hrrchy") == null) return isValidTreeBO; 	// GoTo errExit
            if (pobjBO.getDescriptor().getAttribute("prnt") == null) return isValidTreeBO; 		// GoTo errExit
     ...
     */
     public zXType.rc moveNode(RecTreeNode pobjTree, int pintNode, int pintNewParent) throws ZXException{
         ...
         try {
			...
			while (iter.hasNext()) {
				...
				if (objNode.getBo().getPKValue().compareTo(
						objNodeWhat.getBo().getPKValue()) == 0) {
					...
					/**
					 * And add to children collection of new parent
					 */
     ...
     public zXType.rc moveNodesAccrossTrees(RecTreeNode pobjTree, int pintNode, RecTreeNode pobjNewTree, int pintNewParent) throws ZXException{
         ...
         try {
             /**
              * Simple case if the node to move is actually the root (i.e. move the
              * whole tree
              */
             if (pintNode == pobjTree.getBo().getPKValue().longValue()) {
				...
				/**
				 * Else remove from the child nodes
				 */
				RecTreeNode objNode = pobjTree.findNode(pintNode);
				if (objNode == null) {
					throw new Exception("Unable to find node to move");
				...
				/**
				 * Remove the what-node from the child collection of the old
				 * parent
				 */
				RecTreeNode objNodeWhat;
				...
				while (iter.hasNext()) {
					...
					if (objNode.getBo().getPKValue().compareTo(objNodeWhat.getBo().getPKValue()) == 0) {
						...
						/**
						 * Now add node to children of new parent
						 */
    ...
    public RecTreeNode resolve(ZXBO pobjRoot, String pstrGroup) throws ZXException{
        ...
        try {
            /**
             * Make sure that BO is a valid recursive tree BO
             */
            ...
            /**
             * Retrieve all node records:
             * where hierarchy like '%/pk-of-root/'
             * and root = root-of-root
             * 
             * Note that we have pobjRoot, i.e. the root that the user wants to
             * load and the true root of the tree
             */
            if (objDSHandler.getDsType().pos == zXType.dsType.dstChannel.pos) {
                DSWhereClause objDSWhereClause = new DSWhereClause();
                
                objDSWhereClause.singleWhereCondition(pobjRoot,
                                            pobjRoot.getDescriptor().getAttributes().get("hrrchy"),
                                            zXType.compareOperand.coCNT,
                                            new StringProperty(nodeHierarchyString(pobjRoot) + "/")
                                            , zXType.dsWhereConditionOperator.dswcoNone);
                
                objDSWhereClause.singleWhereCondition(pobjRoot,
                                            pobjRoot.getDescriptor().getAttributes().get("root"),
                                            zXType.compareOperand.coEQ,
                                            pobjRoot.getPKValue(),
                                            zXType.dsWhereConditionOperator.dswcoAnd); // Add to AND statment
                ...
                if (treeHasSequence(pobjRoot)) {
                    ...
                    objRS = objDSHandler.boRS(pobjRoot,
                                      		  pstrGroup, 
                                      		  ':' + objDSWhereClause.getAsWhereClause(),
                                      		  false,
                                      		  "hrrchy,hrrchySrt");
                } else {
                    objRS = objDSHandler.boRS(pobjRoot,
	            ...
	            strQry = strQry + " AND " 
	            		 + getZx().getSql().singleWhereCondition(pobjRoot, 
	                    								pobjRoot.getDescriptor().getAttribute("hrrchy"), 
	                    								zXType.compareOperand.coCNT,
	                    								new StringProperty(this.nodeHierarchyString(pobjRoot) + "/"));
            ...
            if (objRS.eof()) {
            	/**
            	 * Special treatment when no children found
            	 */
            ...
            /**
             * Skip first record and process remainder
             */
            objRS.moveNext();
            ...
            while (!objRS.eof()) {
                ...
                /**
                 * And add to collection of parents
                 */
